...
- **Flow**: (1) MCP validation; (2) fetch story (Jira `getJiraIssue` or GitHub `issue_read`); (3) validate story (description, ≥3 AC; if not, STOP or ask); (4) analyze codebase (codebase_search, grep, patterns); (5) design: for Spec — Blueprint (Context, Architecture, Anti-Patterns) + Contract (DoD, Guardrails, Scenarios); for Plan — Story, Context, Scope, AC, Technical Design, Implementation Steps, Testing, Dependencies, Status; (6) create file: `specs/{FEATURE_DOMAIN}/spec.md` or `.plans/{TASK_KEY}-{kebab}.plan.md`; (7) for Plan: post summary comment to issue (if posting fails, note but do not fail).
- **MCP**: Atlassian (getAccessibleAtlassianResources, getJiraIssue, addCommentToJiraIssue), GitHub (issue_read, add_issue_comment). Optionally ASDLC for pattern lookup.
...
- [ ] **If Spec**: `specs/{FEATURE_DOMAIN}/spec.md` created with Blueprint and Contract. **If Plan**: `.plans/{TASK_KEY}-{kebab}.plan.md` created with required sections; summary posted to issue (or noted if post fails).
